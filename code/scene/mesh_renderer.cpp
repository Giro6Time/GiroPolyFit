...
MeshRenderer::MeshRenderer(std::shared_ptr<Map> mesh, std::shared_ptr<Ui::Canvas> canvas)
    : canvas_(canvas),
      surface_style_(),
      mesh_style_(),
      sharp_edge_style_(),
...
void MeshRenderer::init()
{
    ...
    for (auto it = target_->facets_begin(); it != target_->facets_end(); it++)
    {
        ...
        Logger::warn("NORMAL") << n << std::endl;
        
        std::shared_ptr<Color> c = facet_color_->get((*it)->id);
        Logger::warn("COLOR") << c->r() << ' ' << c->g() << ' ' << c->b() << ' ' << c->a() << std::endl;
        ...
        do
        {
            ...
            normals.push_back(n);
            Logger::warn("VERTICE") << v << std::endl;
    ...
    // 加载shader
    shader_program.addShaderFromSourceCode(QOpenGLShader::Vertex, FileUtils::load_text_source("./shaders/mesh_ver.glsl").c_str());
    bool success = shader_program.addShaderFromSourceCode(QOpenGLShader::Fragment, FileUtils::load_text_source("./shaders/mesh_frag.glsl").c_str());
    if (!success)
    {

        Logger::error("MeshRenderer") << "Failed to load shader!" << std::endl
...
void MeshRenderer::draw_surface()
{
    ...
    {
        Logger::warn("OpenGL Error") << "Error code: " << err << std::endl;
    }
